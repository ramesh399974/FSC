	...
	*/
    ...
    public function actionListAuditPlan()
    {
		...
		if($resource_access != 1){
			...
			if($user_type== 1 && ! in_array('invoice_management',$rules) && ! in_array('audit_management',$rules) ){
				...
				$model = $model->andWhere(['bran.user_id'=>$userid,'bran.status'=>$brand_mod->arrEnumStatus['active']])->andWhere(['appbrand.status'=>$applicationmod->arrBrandEnumStatus['approved'],'tbl_audit.audit_type'=>1]);
				...
				$model = $model->andWhere(['bg.user_id'=>$userid])->andWhere(['appbrand.status'=>$applicationmod->arrBrandEnumStatus['approved'],'tbl_audit.audit_type'=>1,'bran.status'=>$brand_mod->arrEnumStatus['active']]);
			}
			/*
						...
						if(count($appbrandmodel)>0){
							foreach($appbrandmodel as $brmod){
								if($user_type==2){
									...
									if(isset($brmod->brands->user_id) && $brmod->brands->user_id==$userid){
										$brandname[]=$brmod->brands->name;
										$brandgroup[]=$brmod->brands->brandgroup->name;
									}
								}	
	...
	private function canViewAuditPlan($audit_id){
		...
		if($Audit !== null){
			...
			if($Audit->status >= $Audit->arrEnumStatus['submitted']){
				if(Yii::$app->userrole->hasRightsWithFranchise(['assign_as_oss_review_for_tc','view_audit'],$Audit->application->franchise_id)){
					$canViewAudit = 1;
				}
			}
