...
class UserDashboard extends Model
{
    ...
    */
    ...
    public function brandWaitingForApproval($franchiseid,$userid)
	{
		...
		$brand_mod = new Brand();
		$resultarr=array();
		$brdmodel = Brand::find()->where(['user_id'=>$userid,'status'=>$brand_mod->arrEnumStatus['active']])->one();
		...
		if($brdmodel!='' && $brdmodel!==null){
			...
			$brgmodel = BrandGroup::find()->where(['user_id'=>$userid])->one();
			if($brgmodel!='' && $brgmodel!==null){
				$appbrandmod = Application::find()->alias('t');
				$appbrandmod = $appbrandmod->join('inner join','tbl_application_brands as apbr','apbr.app_id=t.id');
				$appbrandmod = $appbrandmod->join('inner join','tbl_brands as br','br.id=apbr.brand_id');
				$appbrandmod = $appbrandmod->join('inner join','tbl_brand_group as bg','br.brand_group_id=bg.id');
				$appbrandmod = $appbrandmod->where(['bg.user_id'=>$userid,'apbr.status'=>array($application->arrBrandEnumStatus['approved']),'br.status'=>$brand_mod->arrEnumStatus['active']])->all();

				if(count($appbrandmod)>0){
					foreach($appbrandmod as $abg){
						$data=array();
						$data['app_id']=$abg['id'];

						$brnamearr=array();
						$brdnamemod = $abg->applicationbrands;
						if(count($brdnamemod)>0){
							foreach($brdnamemod as $brn){
								$brnamearr[]=isset($brn->brands->name)?$brn->brands->name:'';
							}

						}
						$data['brand_name']=implode(', ',$brnamearr);
						$data['brand_group']=$brgmodel->name;
						$data['company_name']=$abg->companyname;
						$resultarr['approved_suppliers'][]=$data;
					}
				}
			}
