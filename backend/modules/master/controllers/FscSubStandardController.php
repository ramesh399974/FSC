...
class FscSubStandardController extends \yii\rest\Controller
{
    ...
    {
        $stds = FscSubStandard::find()->select(['id','fsc_standard_id','name'])->where(['status'=>0])->asArray()->all();
    ...
    public function actionCreate()
    {
        ...
        $model = new FscSubStandard();
		$data = Yii::$app->request->post();
		$responsedata=array('status'=>0,'message'=>'Something went wrong! Please try again');
		if ($data) 
		{
			$chkmodel = FscSubStandard::find()->where(['priority'=> $data['priority']])->one();
			if($chkmodel !== null){
        	...
        	{
                $responsedata=array('status'=>1,'message'=>'FSC Sub Standard has been created successfully');
            }
            else
            {
    ...
    public function actionIndex()
    {
		...
		$stdmodel = new FscSubStandard();
		$model = FscSubStandard::find()->where(['<>','status',2]);
		...
		if(count($model)>0)
		{
			foreach($model as $standard)
			{
                ...
                $data['fsc_standard_id']=$standard->fsc_standard_id;
                $data['fsc_standard_label']=$standard->fscstandard->name;
    ...
    public function actionUpdate()
    {
		...
		if ($data) 
		{            
			$chkmodel = FscSubStandard::find()->where(['!=', 'id', $data['id']])->andWhere(['priority'=> $data['priority']])->one();
			if($chkmodel !== null){
				return $responsedata=array('status'=>0,'message'=>['priority'=>['Priority Already Exists']]);
			}
            ...
            if($model->validate() && $model->save())
			{
				$responsedata=array('status'=>1,'message'=>'FSC Sub Standard has been updated successfully');
			}
			else
			{
				$responsedata=array('status'=>0,'message'=>$model->errors);
    ...
    {
        if (($model = FscSubStandard::findOne($id)) !== null) {
            return $model;
        }
		...
		if ($data && isset($data['status'])) 
		{
			...
			if ($model !== null)
			{
				...
				if($model->validate() && $model->save())
				{
					$msg='';
					if($model->status==0){						
						$msg='FSC Sub Standard has been activated successfully';
					}elseif($model->status==1){						
						$msg='FSC Sub Standard has been deactivated successfully';
                        ...
                        // }
						$msg='FSC Sub Standard has been deleted successfully';
					}
