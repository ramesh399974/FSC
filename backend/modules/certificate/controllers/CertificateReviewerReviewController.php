			...
			if($reviewmodel->validate() && $reviewmodel->save())
			{
				...
				*/
				...
				if($Certificate!==null)
				{
					...
					$appstandards = ApplicationStandard::find()->where(['app_id'=>$application->id])->all();
					$standardids = [];
					...
					if(isset($audit_type) && $audit_type==$appmodel->arrEnumAuditType['renewal']){
						$auditmod = $Certificate->audit->auditplan;
						$audit_completed_date = $auditmod->audit_completed_date;
						$valid_until_date='';
						$renewal_parent_app_id = $application->parent_app_id;

						if(isset($renewal_parent_app_id)){
							...
							$cert_mod = Certificate::find()->where(['parent_app_id'=>$renewal_parent_app_id,'standard_id'=>$standardids,'type'=>$renewals_parent_audit_type])->orderby(['id' => SORT_DESC])->one();
							if($cert_mod!==null){
								$valid_until_date = $cert_mod->certificate_valid_until;
							}

							if(isset($audit_completed_date) && $audit_completed_date!='' && isset($valid_until_date) && $valid_until_date!=''){
								if(strtotime($audit_completed_date) > strtotime($valid_until_date)){
									$appmod = Application:: find()->where(['id'=>$application->id])->one();
									if($appmod!==null){
										$appmod->audit_type = $appmodel->arrEnumAuditType['normal'];
										if($appmod->save()){
											$responsedata['status_change']='Renewal type has been updated successfully';
										}
									}
								}
							}
