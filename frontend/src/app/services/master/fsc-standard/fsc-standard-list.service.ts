...
function sort(standards: Standard[], column: string, direction: string): Standard[] {
  //console.log('234324');
  if (direction === '') {
    return standards;
  } else {
...
export class FscStandardListService {
  private _loading$ = new BehaviorSubject<boolean>(true);
  private _search$ = new Subject<void>();
  private _standards$ = new BehaviorSubject<Standard[]>([]);
  ...
  get fscstandards$() { return this._standards$.asObservable(); }
  ...
  private _search(): Observable<SearchResult> {
    ...
    */
    ...
    return this.http.post<SearchResult>(`${environment.apiUrl}/master/fsc-standard/index`,{page,pageSize,searchTerm,sortColumn,sortDirection}).pipe(
        map(result => {
  ...
  addStandard(standardData){
    ...
    return this.http.post<any>(`${environment.apiUrl}/master/standard/create`, standardData);
    //, JSON.stringify(data)
  }

  getStandard():Observable<any>{
    
    return this.http.get<any>(`${environment.apiUrl}/master/standard/index`,this.httpOptions);
  ...
  commonActionData(data): Observable<any>{
    return this.http.post<any>(`${environment.apiUrl}/master/fsc-standard/common-update`,data);
  } 
}
