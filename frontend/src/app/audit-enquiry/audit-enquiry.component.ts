...
export class AuditEnquiryComponent implements OnInit {
  ...
  fscStanard = [{id: "1", name: "Chain Of Custody", code: "COC"}];
  ...
  */
  ...
  onFscChange(id: number, isChecked: boolean) {
    ...
    //const emailFormArray = <FormArray>this.myForm.controls.useremail;
    //const standardsFormArray = <FormArray>this.enquiryForm.get('company.standardsChk');
    const standardsFormArray = <FormArray>this.enquiryForm.get('fscStd');
    ...
    this.standardsLength = this.enquiryForm.get('fscStd').value.length;
  }
