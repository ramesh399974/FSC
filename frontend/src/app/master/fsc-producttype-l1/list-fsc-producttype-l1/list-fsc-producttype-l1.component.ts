...
import { FscProducttypeL1ListService } from '@app/services/master/fsc-producttype-l1/fsc-producttype-l1-list.service';
...
@Component({
  selector: 'app-list-fsc-producttype-l1',
  templateUrl: './list-fsc-producttype-l1.component.html',
  styleUrls: ['./list-fsc-producttype-l1.component.scss'],
  providers:[FscProducttypeL1ListService]
})
export class ListFscProducttypeL1Component {
  ...
  constructor(public fscproducttypeoneservice: FscProducttypeL1ListService,private modalService: NgbModal,private errorSummary: ErrorSummaryService, private authservice:AuthenticationService) {
    this.producttypes$ = fscproducttypeoneservice.producttypes$;
    this.total$ = fscproducttypeoneservice.total$;
  ...
  onSort({column, direction}: SortEvent) {
    ...
    this.fscproducttypeoneservice.sortColumn = column;
    this.fscproducttypeoneservice.sortDirection = direction;
  }
  ...
  open(content,action,id) {
    ...
    this.fscproducttypeoneservice.commonActionData({id:this.model.id,status:actionStatus}).pipe(first())
      .subscribe(res => {
      this.model.id = null;
      ...
      if(res.status){
        ...
        this.fscproducttypeoneservice.searchTerm=this.fscproducttypeoneservice.searchTerm;
